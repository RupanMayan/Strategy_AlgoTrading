"""
Nifty Bull Put Spread — Interactive HTML Dashboard Generator
...
def generate_dashboard(trades_df: pd.DataFrame, summary: dict, results_dir: Path,
    ...
    for _, row in df.iterrows():
        trade_rows.append({
            ...
            "sell_entry": round(row.get("sell_entry", row.get("entry_price_ce", 0)), 2),
            "buy_entry": round(row.get("buy_entry", row.get("entry_price_pe", 0)), 2),
            "sell_exit": round(row.get("sell_exit", row.get("exit_price_ce", 0)), 2),
            "buy_exit": round(row.get("buy_exit", row.get("exit_price_pe", 0)), 2),
            "credit": round(row.get("net_credit", row.get("combined_premium", 0)), 2),
...
<title>Nifty Bull Put Spread — Backtest Dashboard</title>
...
<h1>Nifty Bull Put Spread — Backtest Dashboard</h1>
    ...
    <div class="table-wrap" id="tableWrap">
        <table>
            <thead><tr>
                ...
                <th data-col="sell_entry">Sell In</th>
                <th data-col="buy_entry">Buy In</th>
                <th data-col="sell_exit">Sell Out</th>
                <th data-col="buy_exit">Buy Out</th>
...
<div class="footer">
    Generated by Nifty Bull Put Spread Backtest Engine — {data['mode']}
</div>
