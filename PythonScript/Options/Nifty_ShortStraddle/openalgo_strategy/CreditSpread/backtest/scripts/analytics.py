...
def compute_summary(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "reentry_trades": int(df["is_reentry"].sum()) if "is_reentry" in df.columns else 0,
        "avg_combined_premium": round(df["combined_premium"].mean(), 2) if "combined_premium" in df.columns else 0,
        "avg_net_credit": round(df["net_credit"].mean(), 2) if "net_credit" in df.columns else 0,
    }
...
"""
    ...
    report = f"""# Backtest Report — Nifty Bull Put Spread
...
| Avg Net Credit | Rs {summary.get('avg_net_credit', 0):,.2f} |
